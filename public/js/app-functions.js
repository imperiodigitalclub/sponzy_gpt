...
(function ($) {
	...
	$(function () {
		$(document).scroll(function () {
			let $nav = $(".scroll");
			let buttonRegister = $('.btn-register-menu');
			$nav.addClass('shadow-custom navbar_background_color p-nav-scroll link-scroll', $(this).scrollTop() > $nav.height());
	...
	$(document).on('click', '#btnCreateUpdate', function (s) {
		...
		var formData = new FormData($('#formUpdateCreate')[0]);
		var isLocked = formData.get('locked');

		console.log("isLocked", isLocked);
		console.log("\nformData:", formData);

		if (!isLocked || isLocked !== 'yes') {
			if (!confirm('Você está postando uma prévia, todos os usuários e visitantes poderão ver o conteúdo. Por favor confirme')) {
				return false;
			}
		}

		// Validação para múltiplos arquivos
		var fileList = formData.get('fileuploader-list-photo');
		var files = fileList ? JSON.parse(fileList) : [];

		if (files.length > 1) {
			$('#modalCarrosselOuMultiplo').modal('show');

			$('#btnCarrossel').on('click', function () {
				$('#createCarousel').val(true);
				$('#modalCarrosselOuMultiplo').modal('hide');
				submitForm();
			});

			$('#btnMultiplo').on('click', function () {
				$('#createCarousel').val(false);
				$('#modalCarrosselOuMultiplo').modal('hide');
				submitForm();
			});
		} else {
		...
		function submitForm() {
			console.log(`formData.get('createCarousel'): ${formData.get('createCarousel')}`);
			...
			// Restante do código de envio do formulário (como no seu exemplo)
