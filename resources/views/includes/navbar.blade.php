<header>
	<nav class="navbar navbar-expand-lg navbar-inverse fixed-top p-nav @if(auth()->guest() && request()->path() == '/') scroll shadow-custom navbar_background_color p-nav-scroll link-scroll @else p-3 @if (request()->is('live/*')) d-none @endif  @if (request()->is('messages/*')) d-none d-lg-block shadow-sm @elseif(request()->is('messages')) shadow-sm @else shadow-custom @endif {{ auth()->check() && auth()->user()->dark_mode == 'on' ? 'bg-white' : 'navbar_background_color' }} link-scroll @endif">
		<div class="container-fluid d-flex position-relative">

		@auth
			<div class="buttons-mobile-nav d-lg-none">
				<a href="{{url('messages')}}" class="btn-mobile-nav position-relative px-1" title="{{ trans('general.messages') }}">
				...
				<a href="{{url('notifications')}}" class="btn-mobile-nav position-relative px-2" title="{{ trans('general.notifications') }}">
		...
		@endauth
			...
			<a class="navbar-brand" href="{{url('/')}}">
				...
				@else
				<img src="{{url('public/img', auth()->guest() && request()->path() == '/' ? $settings->logo_2 : $settings->logo_2)}}" data-logo="{{$settings->logo_2}}" data-logo-2="{{$settings->logo_2}}" alt="{{$settings->title}}" class="logo align-bottom max-w-100" />
			@endif
			</a>
			...
			@endif
				...
				<ul class="navbar-nav ml-auto">
					...
					<li class="nav-item">
						<a @if (Helper::showLoginFormModal()) data-toggle="modal" data-target="#loginFormModal" @endif class="toggleRegister nav-link btn btn-main btn-primary btn-register-menu pr-3 pl-3 btn-arrow btn-arrow-sm" href="{{$settings->home_style == 0 ? url('signup') : url('/')}}">
			...
			@if (auth()->user()->verified_id == 'yes')
				...
				<li class="nav-item dropdown d-lg-none">
                    <a href="{{url('shop')}}" class="nav-link px-2 link-menu-mobile py-1">
                        <div>
                            <i class="feather icon-shopping-bag mr-2"></i>
                            <span class="d-lg-none">{{ __('general.shop') }}</span>
                        </div>
                    </a>
                </li>
			...
			@endif
				...
				<!-- =========== End Menu Mobile ============-->
					...
					<li class="nav-item dropdown d-lg-block d-none">
						<a class="nav-link px-2" href="{{url('/explore')}}" title="{{__('admin.home')}}">
