...
class UpdatesController extends Controller
{
  ...
  public function create()
  {
    ...
    $createCarousel = $this->request->input('createCarousel', false);
    $createCarousel = filter_var($createCarousel, FILTER_VALIDATE_BOOLEAN);
    \Log::info('$createCarousel: ' . $createCarousel);
    ...
    $validator = Validator::make($input, [
      ...
      // Validação condicional para descrição apenas se não houver mídia
      'description' => (!$fileuploader && !$this->request->hasFile('zip') && !$this->request->hasFile('epub') && !$videoUrl) ? 'required|max:' . $this->settings->update_length . '' : 'nullable|max:' . $this->settings->update_length . '',
      '_description' => (!$fileuploader && !$this->request->hasFile('zip') && !$this->request->hasFile('epub') && !$videoUrl) ? 'required|max:' . $this->settings->update_length . '' : 'nullable|max:' . $this->settings->update_length . '',
      'title' => 'max:100',
    ...
    $posts = [];
    \Log::info('count($fileuploader): ' . count($fileuploader));
    \Log::info('Valor de $createCarousel: ' . var_export($createCarousel, true));
    \Log::info('Quantidade de arquivos: ' . count($fileuploader));

    if ($createCarousel == true || count($fileuploader) <= 1) {
      // Criação de um único post, incluindo um carrossel se múltiplos arquivos
      \Log::info('createSinglePost post único');
      $posts[] = $this->createSinglePost($input, $fileuploader, $videoUrl, $statusPost);
    } else {
      // Criação de múltiplos posts para cada arquivo
      \Log::info('createSinglePost post múltiplo');
      foreach ($fileuploader as $file) {
        \Log::info('dentro do for');
        // delay 500ms..
        usleep(500000);
        $posts[] = $this->createSinglePost($input, [$file], $videoUrl, $statusPost);
      }
    }

    // Usando o primeiro post para as verificações e respostas
    $post = $posts[0] ?? null;

    if (!$post) {
      return response()->json([
        'success' => false,
        'errors' => ['error' => __('general.error_creating_post')],
      ]);
    }
  ...
  /**
   * Create a single Update / Post
   *
   * @param array $input
   * @param array $fileuploader
   * @param bool $videoUrl
   * @param string $statusPost
   */
  protected function createSinglePost($input, $fileuploader, $videoUrl, $statusPost)
  {
    \Log::info('createSinglePost dentro da função');
    \Log::info('input: ' . print_r($input, true));
    \Log::info('$this->request->locked: ' . print_r($this->request->locked, true));
    \Log::info('$this->settings->disable_free_post: ' . print_r($this->settings->disable_free_post, true));
    \Log::info('$post->locked: ' . print_r((bool) $this->settings->disable_free_post ? 'yes' : ($this->request->locked ?? 'yes'), true));
    //\Log::info('input: ' . print_r($this->settings, true));
    $post = new Updates();
    $post->description = trim(Helper::checkTextDb($input['description']));
    $post->title = $this->request->locked == 'yes' && !$fileuploader && !$this->request->hasFile('zip') && !$this->request->hasFile('epub') ? $this->request->title : null;
    $post->user_id = auth()->id();
    $post->date = Carbon::now();
    $post->token_id = Str::random(150);
    $post->locked = $this->settings->disable_free_post == 1 ? 'yes' : ($this->request->locked ?? 'no');
    ...
    if ($videoUrl) {
      ...
      Media::create([
        ...
        'image' => '',
        'video' => '',
        'video_embed' => $videoUrl ? $input['urlVideo'] : '',
        'music' => '',
        'file' => '',
        'file_name' => '',
        'file_size' => '',
