...
class UserController extends Controller
{
  ...
  {
    $checkRequest = VerificationRequests::whereUserId(auth()->id())
    ...
    $messages = [
      ...
      "avatar.mimes" => __('validation.mimes', ['attribute' => __('general.avatar')]),
      "cover.mimes" => __('validation.mimes', ['attribute' => __('general.cover')]),
      "document.required" => __('validation.required', ['attribute' => __('general.document')]),
      "cpf.required" => __('validation.required', ['attribute' => __('general.cpf')]),
      "cpf.regex" => __('validation.regex', ['attribute' => __('general.cpf')]),
      "instagram.url" => __('validation.url', ['attribute' => __('general.instagram')]),
    ];
    ...
    $validator = Validator::make($input, [
      ...
      'avatar' => 'nullable|mimes:jpg,jpeg,png|max:' . $this->settings->file_size_allowed_verify_account,
      'cover' => 'nullable|mimes:jpg,jpeg,png|max:' . $this->settings->file_size_allowed_verify_account,
      'birthdate' => 'nullable|date_format:d/m/Y|before:' . Carbon::now()->subYears(18)->format('d/m/Y'),
      'whatsapp' => 'nullable|string|max:15',
      'document' => 'required|string|max:50',
      'cpf' => 'required|string|regex:/^\d{3}\.\d{3}\.\d{3}-\d{2}$/',
      'instagram' => 'nullable|regex:/^@?[a-zA-Z0-9._]+$/|max:255',
    ], $messages);
    ...
    if ($this->request->hasFile('avatar')) {
      $fileAvatar = $this->handleFileUpload('avatar', config('path.avatar'), 'avatar-' . $user->id);
      $user->avatar = $fileAvatar;
    }

    if ($this->request->hasFile('cover')) {
      $fileCover = $this->handleFileUpload('cover', config('path.cover'), 'cover-' . $user->id);
    ...
    // Verifica e atualiza o campo 'documento'
    if (!empty($input['document'])) {
      $user->documento = $input['document'];
    }

    // Verifica e atualiza o campo 'cpf'
    if (!empty($input['cpf'])) {
      $user->cpf = $input['cpf'];
    }

    // Verifica e atualiza o campo 'instagram'
    ...
    // Salva os dados da solicitação de verificação
  ...
  private function handleFileUpload($field, $path, $prefix)
  {
    if ($this->request->hasFile($field)) {
      $photo = $this->request->file($field);
      $extension = $photo->getClientOriginalExtension();
      $filename = strtolower($prefix . '-' . time() . Str::random(40) . '.' . $extension);

      // Verifica se o upload é para avatar ou cover
      if ($field === 'avatar') {
        // Manipula a imagem do avatar
        $imgAvatar = Image::make($photo)->orientate()->fit(200, 200, function ($constraint) {
          $constraint->aspectRatio();
          $constraint->upsize();
        })->encode($extension);
        ...
        Storage::put($path . $filename, $imgAvatar);
        ...
        // Copia a imagem para o diretório
        Storage::put($path . $filename, $imgCover);
