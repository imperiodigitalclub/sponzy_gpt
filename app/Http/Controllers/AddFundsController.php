...
use App\Jobs\AutoCheckPaymentJob;
...
class AddFundsController extends Controller
{
  ...
  public function sendMercadopago()
  {
    try {
      ...
      // External reference
      $preference->external_reference = 'userId=' . auth()->user()->id .
        ',amountOriginal=' . $this->request->amount .
        ',userTaxes=' . auth()->user()->taxesPayable() .
      ...
      // Schedule auto-check of the payment after 2, 10, and 30 minutes
      //$this->scheduleAutoCheckPreference($preference->external_reference, 1);
      $this->scheduleAutoCheckPreference($transactionID, 1, $preference->external_reference);
      $this->scheduleAutoCheckPreference($transactionID, 2, $preference->external_reference);
      $this->scheduleAutoCheckPreference($transactionID, 10, $preference->external_reference);
      $this->scheduleAutoCheckPreference($transactionID, 30, $preference->external_reference);
  ...
  protected function scheduleAutoCheckPreference($transactionID, $minutes, $external_reference)
  {
    $delay = now()->addMinutes($minutes);

    // Job
    \Log::info('Iniciando Job AutoCheckPaymentJob [' . $transactionID . '] [' . $external_reference . '] [' . $delay . ']');
    AutoCheckPaymentJob::dispatch($transactionID, $external_reference)->delay($delay);
  }
